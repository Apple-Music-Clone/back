...
class App<T extends Class<any>[]> {
  ...
  private initializeControllers(controllers: T): void {
    ...
    controllers.forEach((controller) => {
      ...
      const controllersParamsDecorators = Reflect.getMetadata("paramdecorators", controller) ?? new Map();
        ...
        if (method) {        
          ...
          method.call(router, route.path, async (req, res) => {
            ...
            for (const decorator of decorators) {
              ...
              params[decorator.index] = await Promise.resolve(decorator.fn(req, res, decorator.options)); 
            }
            
            try {
              // Pega o retorno da função do controller e salva na constante
              const result = await Promise.resolve(instance[route.key](...params))
